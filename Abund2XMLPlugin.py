...
class graph:
    ...
    def __init__(self, in_gml, style_name, outStyle, abundance_file):
        ...
        def get_abundance_dict(abundance_file, taxa_id_dict):
        ...
        def map_taxa_id(graph_dict):
            taxa_id_dict={}
            for id in graph_dict.keys():
                taxa_id_dict[graph_dict[id]["label"]] = id
            return taxa_id_dict
    ...
    def add_node_type(self, keyword_type_set):
        prop = "NODE_SHAPE"
        default = "ELLIPSE"

        def get_node_type_dict(abundance_file, keyword_type_set={("HMDB","DIAMOND")}):
            ab_df = pd.read_csv(abundance_file, index_col=0)
            taxa_list = list(ab_df.columns)
            type_dict = {}
            for taxa in taxa_list:
        ...
        type_dict = get_node_type_dict(self.abundance_file, keyword_type_set)

        self.add_property_node(prop, type_dict, default="ELLIPSE")
    ...
    def fix_node_label(self, metadata):
...
class Abund2XMLPlugin:
    ...
    def output(self, outfile):
        in_gml = PyPluMA.prefix()+"/"+self.parameters["gmlfile"]
        styleName = self.parameters["styleName"]
        outStyle = outfile
        abundance_file = PyPluMA.prefix()+"/"+self.parameters["abundance"]

        G = graph(in_gml, styleName, outStyle, abundance_file)
        G.add_node_size()
        G.add_node_type({("HMDB", "DIAMOND")})
        G.close_node()
        G.fix_node_label(PyPluMA.prefix()+"/"+self.parameters["metadata"])
        G.write_style()
